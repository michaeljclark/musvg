...
typedef int (*size_check_fn)(vf_buf*,size_t);
...
static inline int vf_buf_check_capacity(vf_buf *buf, size_t len)
{
    return (buf->data_offset + len > buf->data_size) ? -1 : 0;
}

static inline int vf_buf_ensure_capacity(vf_buf *buf, size_t len)
{
    if (buf->data_offset + len > buf->data_size) {
        size_t old_size = buf->data_size;
        do {
            buf->data_size <<= 1;
        } while (buf->data_offset + len > buf->data_size);
        buf->data = (char*)realloc(buf->data, buf->data_size);
        memset(buf->data + old_size, 0, buf->data_size - old_size);
    }
    return 0;
}
...
static void* vf_buf_get_userdata(vf_buf *buf) { return buf->user_data; }
static void vf_buf_set_userdata(vf_buf *buf, void *user_data) { buf->user_data = user_data; }
...
{                                                                              \
    if (buf->cb(buf, sizeof(T))) return 0;                                     \
    T t = swap(val);                                                           \
...
{
    if (buf->cb(buf, strlen(fmt)*2)) return 0;

    size_t remaining = buf->data_size - buf->data_offset;

    va_list args;
    va_start(args, fmt);
    int len = vsnprintf(buf->data + buf->data_offset, remaining, fmt, args);
    va_end(args);
    if ((size_t)len > remaining) {
        if (buf->cb(buf, len)) return 0;
        remaining = buf->data_size - buf->data_offset;
        va_list args;
        va_start(args, fmt);
        len = vsnprintf(buf->data + buf->data_offset, remaining, fmt, args);
        va_end(args);
    }
    buf->data_offset += len;
    return len;
